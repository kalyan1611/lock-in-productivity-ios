...
final class NetworkManager: ObservableObject {
    ...
    func sendSync(steps: Int, gymSeconds: Int) async throws -> SyncResult {
        ...
        do {
            ...
            self.goalMet = result.goalMet
            self.stepGoalMet = result.stepGoalMet
            self.gymGoalMet = result.gymGoalMet
    ...
    func checkStatus() async {
        do {
            ...
            if let decoded = try? JSONDecoder().decode(StatusResult.self, from: data) {
                self.goalMet = decoded.overallGoalMet
                self.stepGoalMet = decoded.stepGoalMet
                self.gymGoalMet = decoded.gymGoalMet
            }
        } catch {
            connectionStatus = .offline
        }
