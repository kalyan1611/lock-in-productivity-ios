...
final class LeetCodeManager: ObservableObject {
    ...
    private let graphqlEndpoint = URL(string: "https://leetcode.com/graphql")!
    ...
    func fetchTodaySolvedProblems() async {
        ...
        do {
            ...
            self.easyTodayCount = easy
            self.mediumTodayCount = medium
            self.hardTodayCount = hard
            self.lastUpdated = Date()
            self.isLoading = false
        } catch {
            self.errorMessage = "Failed to update LeetCode data"
            self.isLoading = false
        }
    }
